...
class News extends CI_Controller {
	...
	public function cat($filter1='', $filter2='', $filter3='', $kategori_id='')
	{
		$where_account['account_email']		= $this->session->userdata('account_email');
		$data['account']					= $this->ADM->get_account('',$where_account);  
		$where['kategori_id'] 			= $filter1;
		...
		$data['jml_data']					= $this->ADM->count_all_berita($where,'');
		$data['jml_halaman'] 				= ceil($data['jml_data']/$data['batas']);
		$row = $this->ADM->get_berita('*', $where_kategori);
		error_reporting(0);
		$data['title']						= $row->kategori_judul. ' |';
		$data['action']						= 'pengumuman';
		$data['slide']				= TRUE;
		$data['boxtodaydeals']		= TRUE;
		$data['boxkategori']		= TRUE;
	...
	public function read($berita_id='', $filter2='', $berita_judul='')
	{
	  ...
	  if($get == "")
	  {
		...
		$data['title']					= $row->kategori_judul. ' > ' .$row->berita_judul.' |';
		$data['action']					= 'detail';
